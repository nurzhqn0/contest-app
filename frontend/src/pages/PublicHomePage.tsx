import { ArrowRight, GlobeHemisphereWest, Key, Medal, ShieldCheck, TelegramLogo } from "@phosphor-icons/react";
...
const publicSteps = [
  {
    ...
    description: "They start the bot, enter the room code, add a name, and receive today’s task prompts.",
    icon: TelegramLogo
  },
  {
    title: "Guests open the public room page",
    description: "The same room code opens the public leaderboard when the organiser allows public access.",
    ...
    <main className="page-shell grid min-h-[100dvh] gap-6 py-6 md:py-8 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            <h1 className="headline-balance max-w-3xl text-4xl font-semibold tracking-[-0.05em] text-ink md:text-6xl">
              Open a room by code and view the challenge as a participant or guest.
            </h1>
            <p className="copy-pretty max-w-[64ch] text-base leading-relaxed text-muted md:text-lg">
              Student Contest supports two user paths: Telegram participants submit daily progress, while public viewers
              can follow the leaderboard from the web when access is enabled.
            </p>
          </div>
        </div>
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-4">
            {publicSteps.map((item, index) => (
              <article key={item.title} className="reveal rounded-lg border border-line bg-[#f7f6f2]/85 p-4" style={{ ["--index" as string]: index + 1 }}>
                <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-lg bg-white text-accent">
                  <item.icon size={20} weight="duotone" />
                </div>
                <h2 className="text-lg font-semibold tracking-[-0.03em] text-ink">{item.title}</h2>
          ...
          <article className="surface-muted flex flex-col justify-between p-5">
            <div className="space-y-3">
              ...
              <div className="rounded-lg border border-line bg-white p-4">
                ...
                <div className="mono-data text-2xl font-semibold tracking-[0.14em] text-ink">RAM25</div>
              </div>
              <div className="rounded-lg border border-line bg-white p-4">
                <div className="mb-2 flex items-center gap-2 text-sm font-medium text-ink">
                ...
                <div className="mono-data break-all text-sm text-muted">yourapp.com/room/RAM25</div>
              </div>
            </div>
      ...
      <section className="surface reveal flex flex-col justify-between p-6 md:p-8" style={{ ["--index" as string]: 1 }}>
        <div className="space-y-4">
          ...
          <h2 className="text-3xl font-semibold tracking-[-0.04em] text-ink md:text-4xl">Enter the room code</h2>
          <p className="copy-pretty max-w-[56ch] text-sm leading-relaxed text-muted">
            If the code is valid and public access is enabled, you will open the room page with the latest leaderboard
            and active tasks.
          </p>
        </div>
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-2">
          <div className="surface-muted p-4">
            ...
            <p className="mt-2 text-sm leading-relaxed text-muted">Use Telegram to register, submit daily tasks, and view your rank.</p>
          </div>
          <div className="surface-muted p-4">
            <div className="eyebrow">For guests</div>
            <p className="mt-2 text-sm leading-relaxed text-muted">Use the code on this page to watch the public leaderboard in real time.</p>
          </div>
        </div>
      </section>
    </main>
