...
export function PublicRoomPage() {
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket(
      `${WS_BASE}/ws/rooms/${roomQuery.data.id}/leaderboard`,
    );
    socket.onmessage = () =>
      queryClient.invalidateQueries({ queryKey: ["public-room", roomCode] });
    return () => socket.close();
          ...
          <EmptyState
            ...
            description={
              roomQuery.error instanceof Error
                ...
                : "The requested room could not be opened."
            }
            action={
        ...
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-5">
            ...
            <p className="copy-pretty max-w-[68ch] text-base leading-relaxed text-muted">
              {room.description ||
                "The organiser has not added a room description yet."}
            </p>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
            ...
            <article className="surface-muted p-4">
              ...
              <p className="mt-2 text-sm leading-relaxed text-muted">
                Leaderboard refreshes automatically while this page stays open.
              </p>
            </article>
          </div>
        </div>
          ...
          {room.tasks.length ? (
            <div className="space-y-3">
              {room.tasks.map((task, index) => (
                <article
                  key={task.id}
                  className="reveal rounded-lg border border-line bg-[#f7f6f2]/7 p-4"
                  ...
                  <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        />
                        <StatusBadge
                          label={`Target ${formatTaskTarget(task)}`}
                        />
                      </div>
                    </div>
                    <div className="space-y-2 text-left md:text-right">
                      <div className="mono-data text-lg font-semibold text-ink">
                        {task.points} pts
                      </div>
